...
export class BasicPitchProvider implements MidiProvider {
  name = "basic-pitch";
  private apiUrl = process.env.BASIC_PITCH_API_URL || null;
  ...
  async generateMidi(
    ...
    if (!this.apiUrl) {
      return { success: false, error: "Basic Pitch API URL not configured" };
    }
  ...
  async generateMidi(
    ...
    try {
      ...
      if (status !== "completed" || !result.midi_url) {
        return {
          success: false,
          error: `MIDI generation failed with status: ${status}`,
        };
      }
...
export class MidiGenerationManager {
  ...
  constructor() {
    this.providers = [new BasicPitchProvider(), new FadrMidiProvider()];
  }
  ...
  async generateMidi(
    ...
    for (const provider of this.providers) {
      if (provider.isConfigured()) {
        const result = await provider.generateMidi(
          audioPath,
          outputDir,
          stemType,
        );
  ...
  async generateMidiForStems(
    stems: Array<{ type: string; path: string }>,
    outputDir: string,
  ): Promise<MidiGenerationResult> {
