...
import {
  identifyTrack,
  lookupBySpotifyId,
  lookupByAppleMusicId,
  extractTrackInfo,
} from "../services/songlink.js";
...
const ENABLE_CATALOG_ACQUISITION =
  process.env.ENABLE_CATALOG_ACQUISITION === "true";
...
async function acquireAudioStep(job: Job): Promise<Partial<Job>> {
  ...
  if (!ENABLE_CATALOG_ACQUISITION) {
    throw new Error(
      "Catalog audio acquisition is disabled for TuneForge v1. Upload or record audio you own to create stems and MIDI.",
    );
  }
  ...
  if (platformIds.deezerId || platformIds.tidalId || platformIds.qobuzId) {
    ...
    const result = await providerManager.downloadByPlatformIds(
      platformIds,
      outputPath,
    );
  ...
  throw new Error(
    "Audio not available - no FLAC provider configured. Configure Tidal, Deezer, or Qobuz API keys in Settings.",
  );
}
...
async function separateStemsStep(job: Job): Promise<Partial<Job>> {
  ...
  const result = await stemManager.separateStems(
    job.masterAudioPath,
    outputDir,
  );
  ...
  for (const stem of result.stems) {
    const midi = result.midiFiles?.find((file) => file.stemType === stem.type);
...
async function generateMidiStep(job: Job): Promise<Partial<Job>> {
  ...
  const stems = jobAssets
    .filter((a) => a.type === "stem" && a.stemType && !a.hasMidi)
  ...
  if (process.env.ENABLE_MIDI_FALLBACK !== "true") {
    return {};
  }
...
const catalogProcessingSteps: ProcessingStep[] = [
...
const uploadProcessingSteps: ProcessingStep[] = [
  { name: "separating", progress: 20, execute: separateStemsStep },
  { name: "generating_midi", progress: 90, execute: generateMidiStep },
];
...
export async function processJob(jobId: string): Promise<void> {
  ...
  try {
    let steps = catalogProcessingSteps;

    if (job.sourceType === "file_upload") {
      const uploadUpdates = await acquireAudioStep(job);
      if (Object.keys(uploadUpdates).length > 0) {
        await db
          .update(jobs)
          .set({ ...uploadUpdates, updatedAt: new Date() })
          .where(eq(jobs.id, jobId));

        job = { ...job, ...uploadUpdates } as Job;
      }
    ...
    const expiresAt = new Date(
      Date.now() + ASSET_EXPIRY_HOURS * 60 * 60 * 1000,
    );
